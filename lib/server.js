...
module.exports = function (options) {
  ...
  var server = http.createServer(function (req, res) {
    ...
    href(req, res, function () {
      ...
      if (!vhost) {
        if (options.verbose) log.request(req.ip, 'NULL', req.method, req.url, req.headers);
        res.writeHead(404);
        return res.end();
      }
      ...
      if (vhost.file) {
        return dish.file(dgate.tokens(vhost.file, req))(req, res, function (err) {
  ...
  server.on('upgrade', function (req, socket, head) {
    var vhost = dgate.match(server.vhosts, req);
    if (vhost) {
      if (options.verbose) log.request(req.ip, vhost.target, 'UPGRADE', req.url, req.headers);
